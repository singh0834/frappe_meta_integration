...
def get_attach_link(doc, print_format):
    ...
    url = get_url() + "/api/method/frappe_meta_integration.whatsapp.pdf_utils.get_print_pdf?key=" + doc.get_signature() + "&doc="+doc.doctype + "&name="+ doc.name + "&printf=" + print_format
    return url

@frappe.whitelist(allow_guest=True)    
def get_print_pdf(key, doc, name, printf):
    # frappe.log_error("error", [key, doc, name, printf])
    if not key == frappe.get_doc(doc, name).get_signature():
        frappe.log_error("heelo")
        return
    
    html = p(doc, name, printf)
    ...
    # Corrected: Access metadata as a dictionary, no need for parentheses
    metadata = reader.metadata  # Updated here

    # Updated method name to add_metadata (instead of addMetadata)
    writer.add_metadata(metadata)
    
    # Optionally, add custom title metadata
    writer.add_metadata({
