...
# Get Access Token from WhatsAPP Cloud API Setting
...
# Send Welcome message:--> Need to check whether this is reuired or not.
...
# Get All WhastApp Contact List which is already saved in the ist time.
...
# This is the actual method for the sending WhatsApp Template and Message.

@frappe.whitelist()
def send_whatsapp_msg(doctype, docname, args, template_parameter):
	"""
	...
	template = args['whatsapp_template']
	...
	if args.get('header_1_attachment') and args['header_1_attachment']: #If Attachment included
		doctype = doctype
		docname =  docname
		title =  docname
	...
	# if not args['header_1_attachment'] and not message:
	# 	frappe.throw(
